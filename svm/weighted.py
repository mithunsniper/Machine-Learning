...
for C in Cs:
    for g in gamma_range:
        ...
        svc =SVC(kernel='rbf', gamma=g,C=C,class_weight='balanced')
...
svclassifier = SVC(kernel='rbf',C=b[0][0],gamma=b[0][1],class_weight='balanced')  
start = time.time()
...
for g in gamma_ranges:
    for p in pol:
        for c in Css:
            svc =SVC(kernel='poly',degree=p,C=c,gamma=g,class_weight='balanced')
...
svclassifier = SVC(kernel='poly',C=b[0][2],gamma=b[0][0],degree=b[0][1],class_weight='balanced')  
start = time.time()
svclassifier.fit(x_train, y_trainlabel) 
end =time.time()
y_pred = svclassifier.predict(x_test)
...
svclassifier = SVC(kernel='linear',C=b[0][0],class_weight='balanced')  
start = time.time()
svclassifier.fit(x_train, y_trainlabel) 
end =time.time()
y_pred = svclassifier.predict(x_test)
...
for co in coeff:
    for g in gamma_range:
        ...
        for c in Cs:
            svc =SVC(kernel='sigmoid', gamma=g,coef0=co,C=c,class_weight='balanced')
...
svclassifier = SVC(kernel='sigmoid',C=b[0][2],gamma=b[0][1],coef0=b[0][0],class_weight='balanced')  
start = time.time()
svclassifier.fit(x_train, y_trainlabel) 
end =time.time()
y_pred = svclassifier.predict(x_test)
print("Overall Accuracy (sigmoid) : ",accuracy_score(y_testlabel,y_pred)*100)
